         ...
         System.out.println(attPokemon.getPokeName()+" uses "+
          ...
          // Confused Damage Algorithm
           pokeMove.confusedDamage = (int) Math.round(
            ...
            //exception handling for if pokemon attack stage is max
            if(targetPokemon.getAttackStage()==12){
                System.out.println("Attack level is at maximum, so it doesnt increase the attack anymore");
            }
            else{
            targetPokemon.setStage(targetPokemon.getStage()+2);
            targetPokemon.setAttack
    (stage_multiplier[targetPokemon.getStage()] * targetPokemon.getBaseAttack()); }
